...
```{r}
llegadas_regiones <- read_file_srv("/economia2/anuario/llegadas turistas internacionales_OCT_24.xlsx",skip=1,sheet=2) %>% 
  rename(region = 1) %>% 
  mutate(region = ifelse(region == "Américas","América",region))
ingresos_regiones <- read_file_srv("/economia2/anuario/ingresos turistas internacionales_OCT_24.xlsx",skip=1,sheet=2) %>% 
  rename(region = 1) %>% 
  mutate(region = ifelse(region == "Américas","América",region))

llegadas_subregiones <- read_file_srv("/economia2/anuario/llegadas turistas internacionales_OCT_24.xlsx",skip=1,sheet=3) %>% 
  rename(subregion=1) %>% 
  mutate(region = ifelse(region == "Américas","América",region))

ingresos_subregiones <-  read_file_srv("/economia2/anuario/ingresos turistas internacionales_OCT_24.xlsx",skip=1,sheet=3) %>% 
  rename(subregion=1) %>% 
  mutate(region = ifelse(region == "Américas","América",region))
...
data_llegadas_mundo %>% 
  ...
  labs(fill = "", x = "", y = "", caption = "Fuente: DNMyE en base a ONU Turismo") +
  theme_minimal() +
...
data_tabla_subregion <- llegadas_subregion_long %>%  
  ...
  filter(!is.na(`2000`) & subregion != "Mundo") %>%
    select(subregion,`2000`,`2005`,`2010`,`2015`,`2022`,`2023`) %>% mutate_at(vars(-subregion),as.numeric))
...
tabla_omt_subregion <- data_tabla_subregion %>% 
  select(region,subregion,`2000`,`2005`,`2010`,`2015`,`2022`,llegadas,var_i_a,var_2019,part) %>% 
  group_by(region) %>% 
  gt() %>% 
 ...
 tab_source_note(
    source_note = md(
      "**Fuente**: DNMyE en base a ONU Turismo")
  ) %>% 
...
data_tabla_paises <- llegadas_paises %>% 
  ...
  ungroup() %>% 
  filter(anio %in% c(2000,2005,2010,2015,2022,anio_actual)) %>% 
  select(nombre,anio,valor,var_i_a,var_2019) %>% 
  pivot_wider(names_from = anio,values_from = c(valor,var_i_a,var_2019))
```

Las llegadas de turistas internacionales en **`r data_tabla_paises %>% arrange(-valor_2023) %>% slice(1) %>% pull(nombre)`** `r  ifelse(data_tabla_paises %>% arrange(-valor_2023) %>% slice(1) %>% pull(var_i_a_2023) >0,"crecieron",cayeron)` `r round(data_tabla_paises %>% arrange(-valor_2023) %>% slice(1) %>% pull(var_i_a_2023) %>% abs() *100,1 )`%. En comparación con 2019, la variación fue de `r round(data_tabla_paises %>% arrange(-valor_2023) %>% slice(1) %>% pull(var_2019_2023)  *100,1 )`%. El siguiente país en magnitud fue **`r data_tabla_paises %>% arrange(-valor_2023) %>% slice(2) %>% pull(nombre)`**, que `r  ifelse(data_tabla_paises %>% arrange(-valor_2023) %>% slice(2) %>% pull(var_i_a_2023) >0,"aumentó","disminuyó")` sus llegadas un `r round(data_tabla_paises %>% arrange(-valor_2023) %>% slice(2) %>% pull(var_i_a_2023) %>% abs() *100,1 )`% con respecto a `r as.character(anio_actual-1)` (`r round(data_tabla_paises %>% arrange(-valor_2023) %>% slice(2) %>% pull(var_2019_2023)  *100,1 )`% vs. 2019). Por su parte, las llegadas a **`r data_tabla_paises %>% arrange(-valor_2023) %>% slice(3) %>% pull(nombre)`** `r  ifelse(data_tabla_paises %>% arrange(-valor_2023) %>% slice(3) %>% pull(var_i_a_2023) >0,"se elevaron","se contrajeron")` un `r round(data_tabla_paises %>% arrange(-valor_2023) %>% slice(3) %>% pull(var_i_a_2023) %>% abs() *100,1 )`% (`r round(data_tabla_paises %>% arrange(-valor_2023) %>% slice(3) %>% pull(var_2019_2023)  *100,1 )`% vs. 2019).


```{r tabla-omt-paises, fig.cap=""}
tabla_llegadas_paises <- data_tabla_paises %>%
  select(nombre,valor_2000,valor_2005,valor_2010,valor_2015,valor_2022,valor_2023,var_i_a_2023,var_2019_2023) %>% 
  mutate_at(vars(-nombre),as.numeric) %>% 
  arrange(-valor_2023) %>% 
  slice(1:20) %>% 
  gt() %>%
  # fmt_number(columns=c(valor_2000,valor_2005,valor_2010,valor_2015,valor_2020,valor_2022),decimals = 1,dec_mark = ",") %>% 
  gt_theme_dnmye() %>% 
  tab_caption("Llegadas de turistas internacionales. En millones. Primeros 20 países.") %>% 
  tab_spanner(columns = c(valor_2023,var_i_a_2023,var_2019_2023),
               label = md(glue("**{anio_actual}**"))) %>% 
  cols_label(nombre = md(""),
             ...
             valor_2015 = md("2015"),
             valor_2022 = md("2022"),
             valor_2023 = md("Llegadas"),
             var_i_a_2023 = md("Var. i.a. (%)"),
             var_2019_2023 = md("Var. vs. 2019 (%)")) %>%
  tab_style(
    style = cell_text(weight =  "bold"),
    locations = cells_row_groups()
  ) %>% 
...
data_graf_llegadas_paises <- 
 ...
 filter(nombre %in% ( data_tabla_paises %>% arrange(-valor_2023) %>% slice(1:10) %>% pull(nombre))) 
```
...
tabla_ingresos_omt_subregion <- data_tabla_ingresos_subregion %>% 
  select(region,subregion,`2000`,`2005`,`2010`,`2015`,`2022`,ingresos,var_i_a,var_2019,part) %>% 
  group_by(region) %>% 
  gt() %>% 
   gt_theme_dnmye()%>% 
  fmt_percent(columns = c(part),decimals = 1, sep_mark = ".",dec_mark = ",") %>% 
...
ranking_ingresos_ultimo %>% 
  ...
  scale_y_continuous(labels = scales::number_format(scale = 1/1e+6))+
  scale_fill_dnmye(discrete = "F") +
  labs(fill = "", x = "", y = "", caption = "**Fuente:** DNMyE en base ONU Turismo") +
  theme_minimal() +
  theme(legend.position = "none",
        plot.caption  = element_markdown())
...
tabla_ingresos_paises <- data_tabla_ingresos_paises %>%
  ...
  slice(1:20) %>% 
  gt() %>% 
  # fmt_number(columns = c(valor_2000,valor_2005,valor_2010,valor_2015,valor_2020,valor_2022),
...
data_graf_ingresos_paises <-   ingresos_paises %>% 
 ...
 filter(nombre %in% (data_tabla_ingresos_paises %>% arrange(-valor_2023) %>% slice(1:10) %>% pull(nombre))) 
...
tabla_egresos_paises <- data_tabla_egresos_paises %>%
  ...
  gt_theme_dnmye() %>% 
  tab_spanner(columns = c(valor_2022,var_i_a_2023,var_2019_2023),
               label = md(glue("**{anio_actual}**"))) %>% 
  cols_label(nombre = md(""),
             valor_2000 = md("2000"),
             valor_2005 = md("2005"),
...
data_graf_egresos_paises <-  egresos_paises %>% 
    mutate(valor = as.numeric(valor),
         anio = as.numeric(anio)) %>% 
  filter(indicador == "Travel" & anio > 2004) %>%
 filter(nombre %in% (data_tabla_egresos_paises %>% arrange(-valor_2023) %>% slice(1:10) %>% pull(nombre))) 
...
tabla_llegadas_america <- data_tabla_america %>%
  select(pais,llegadas_2005,llegadas_2010,llegadas_2015,llegadas_2022,llegadas_2023,var_i_a_2023,var_2019_2023) %>% 
  mutate_at(vars(-pais),as.numeric) %>% 
  arrange(-llegadas_2023) %>% 
  slice(1:20) %>% 
  gt() %>% 
  # fmt_number(columns=c(llegadas_2005,llegadas_2010,llegadas_2015,llegadas_2020,llegadas_2022),decimals = 1,dec_mark = ",") %>% 
  gt_theme_dnmye() %>% 
  tab_caption("América: llegadas de turistas internacionales. En millones. Primeros 20 países.") %>% 
  tab_spanner(columns = c(llegadas_2023,var_i_a_2023,var_2019_2023),
               label = md(glue("**{anio_actual}**"))) %>% 
  cols_label(pais = md(""),
             llegadas_2005 = md("2005"),
             llegadas_2010  = md("2010"),
             llegadas_2015 = md("2015"),
             llegadas_2022 = md("2022"),
             llegadas_2023 = md("Llegadas"),
             var_i_a_2023 = md("Var. i.a. (%)"),
             var_2019_2023 = md("Var. vs. 2019 (%)")) %>%
  tab_style(
    style = cell_text(weight =  "bold"),
...
tabla_ingresos_america <- data_tabla_america_ingresos %>%
  select(pais,ingresos_2005,ingresos_2010,ingresos_2015,ingresos_2022,ingresos_2023,var_i_a_2023,var_2019_2023) %>% 
  mutate_at(vars(-pais),as.numeric) %>% 
  arrange(-ingresos_2022) %>% 
  slice(1:20) %>% 
  gt() %>% 
  # fmt_percent(columns=c(var_i_a_2022,var_2019_2022),decimals = 1,dec_mark = ",") %>% 
  gt_theme_dnmye() %>% 
  fmt_number( columns = c(ingresos_2005,ingresos_2010,ingresos_2015,ingresos_2022,ingresos_2023), decimals = 0, dec_mark = ",", sep_mark = ".") %>% 
  tab_caption("América: ingresos por turismo internacional. En millones de dólares. Primeros 20 países.") %>% 
  tab_spanner(columns = c(ingresos_2023,var_i_a_2023,var_2019_2023),
               label = md(glue("**{anio_actual}**"))) %>% 
  cols_label(pais = md(""),
             ingresos_2005 = md("2005"),
             ingresos_2010  = md("2010"),
             ingresos_2015 = md("2015"),
             ingresos_2022 = md("2022"),
             ingresos_2023 = md("Ingresos"),
             var_i_a_2023 = md("Var. i.a. (%)"),
             var_2019_2023 = md("Var. vs. 2019 (%)")) %>%
  tab_style(
    style = cell_text(weight =  "bold"),
