...
[^nota_empalme]:Tanto en 2013 como en 2016 dicho organismo realizó mejoras en las estimaciones de los datos, gracias a un conjunto de revisiones metodológicas y a una mayor explotación de los datos migratorios. Para poder tener una serie comparable, desde la DMyE se realizó un empalme de la serie a partir de las variaciones trimestrales por medio de transporte y país de la serie anterior. 

En la primera sección, inicialmente, se presentarán  los indicadores más importantes acerca de la evolución general de los **viajes de turistas** residentes y no residentes en base a los datos de la DMyE. Luego, se mostrará información sobre el **gasto y estadía** con datos de  la DNESEyCI del INDEC.
...
#Receptivo: anuales (1990-anio anuario) por pais_agrupado (se levanta base hasta año 2015)

receptivo_anual_pais_1990_2015 <- read_file_srv("/srv/DataDNMYE/turismo_internacional/bases_proceso/series/serie_anual_pais_1990.xlsx", 
                                 sheet = "receptivo") %>% 
  filter(anio<=2015)

# Se utiliza la base de visitantes para generar los años a partir de 2016:

receptivo_2016_act <- base_visitantes %>% 
  filter(turismo_internac=="Receptivo",tipo_visitante=="Turistas",anio>2015,anio<=anio_ref) %>% 
  group_by(anio,pais_agrupado) %>% 
  summarise(viajes_turistas=sum(casos_ponderados))

receptivo_anual_pais <-  rbind(receptivo_anual_pais_1990_2015,receptivo_2016_act)

rm(receptivo_anual_pais_1990_2015,receptivo_2016_act)
...
emisivo_anual_pais <- rbind(emisivo_90_2015, emisivo_2016 %>% filter(anio>=2016)) 
...
serie_vis_r <-excursionistas %>% 
  ...
  bind_rows(serie_vis_r_actual %>% filter(anio>=2016)) %>% 
...
tab_source_note(source_note = md("**Fuente**: DMyE en base a datos de DNM y ETI.")) %>% 
...
Como se puede observar, el año 2023 evidencia una recuperación del turismo receptivo luego del impacto generado por la pandemia, alcanzando valores similares a los del 2019 (el registro más elevado).
...
graf_1.1_anual <- ggplot(receptivo_anual_lim_final, aes(ym(paste(anio, 1, sep = "-")), tur/1000000, colour = limitrofe)) +   
  ...
  labs(y = "en millones de viajes de turistas", 
       x = "", 
       color= "",
       caption =  "**Fuente**: DMyE en base a datos de DNM y ETI." )
...
lbl_decimal(pull(g_1.3_texto_r[1,"porc"]*100))`% del total), seguido por `r g_1.3_texto_r[2,"pais_agrupado"]` y  `r g_1.3_texto_r[3,"pais_agrupado"]` con `r 
...
`r lbl_decimal(pull(g_1.3_texto_r[3,"porc"]*100))`% de participación, respectivamente.  
...
geom_label(aes(label = format(round(tur,0), big.mark = ".", decimal.mark = ","), group = anio),
...
Al observar la evolución mensual del año `r as.character(anio_ref)` se puede ver una tendencia de recuperación sostenida con respecto al año anterior, con fuertes variaciones positivas en los meses estivales que luego se fueron moderando a lo largo del año.
...
texto_1 <- via_pais_con_v19 %>% 
  filter(pais_agrupado=="Total") %>% 
  select(starts_with("prop")) %>% 
  pivot_longer(cols = 1:3, names_to = "indicador",
               values_to = "valor") %>% 
  arrange(desc(valor)) %>% 
  mutate(indicador=case_when(indicador=="prop_terrestre"~"terrestre",
                             indicador=="prop_aereo"~"aérea",
                             indicador=="prop_fluvial_maritimo"~"fluvial/marítima"))

texto_2 <- via_pais_con_v19 %>% 
  filter(pais_agrupado=="Total") %>% 
  select(var_aereo,var_fluvial_maritimo,var_terrestre) %>% 
  pivot_longer(cols = 1:3, names_to = "indicador",
               values_to = "valor") %>% 
  arrange(desc(valor)) %>% 
  mutate(indicador=case_when(indicador=="var_terrestre"~"terrestre",
                             indicador=="var_fluvial_maritimo"~"fluvial/marítima",
                             indicador=="var_aereo"~"aérea"))
...
ti6 <- gt(via_pais_con_v19) %>% 
  ...
  tab_source_note(
    source_note = md("**Fuente**: DMyE en base a datos de DNM y ETI.")) %>%
  tab_options(container.height = 600,
              container.overflow.y = T) %>%
...
El análisis desagregado según medio de transporte arroja que en `r as.character(anio_ref)` la principal vía de ingreso fue la `r texto_1[1,1]` (concentrando el `r lbl_percent(as.numeric(texto_1[1,2]))`, con una var.i.a de `r lbl_percent(as.numeric(texto_2[1,2]))`), seguida por la vía `r texto_1[2,1]`.
...
cuenta_viajes_ori <- read_file_srv("/DataDNMYE/cuentas_internacionales/base_detallada/base_detallada_viajes_mintur_20241T.xlsx") %>% 
  janitor::clean_names() %>% 
  rename(anio = ano) %>% 
...
cuenta_pasajes <- read_file_srv("/DataDNMYE/cuentas_internacionales/base_detallada/pasajes_MT_20234T.xlsx") %>% 
  janitor::clean_names() %>% 
  ...
  summarise(receptivo_p = sum(monto[cod_bde_p %in% c("1Ab0311",# marítimo pasajeros
            ...
            emisivo_p = sum(monto[cod_bde_p %in% c("1Ab0311",# marítimo pasajeros
                                                               ...
                                                               "1Ab0331") # otras modalidades pasajeros
                                              & cd_bdd_p == "d"])) %>% 
   filter (anio >2019, anio<=anio_ref )#los datos de 2019 de pasajes están en base anterior. 
...
fmt_number(columns  = 1, sep_mark = "", decimals = 0) %>% 
tab_source_note(source_note = md("**Fuente**: DMyE en base a datos de INDEC.")) %>% 
  tab_options(container.height = 600,
              container.overflow.y = T) %>%
...
En 2023, se observa un fuerte incremento vs 2022, alcanzando valores pre-pandemia (tanto en el gasto de turistas de países limítrofes como no limítrofes).  
...
ti8 <- ggplot(gasto_lim_rec, aes(ym(paste(anio, 1, sep = "-")), gasto/1000000, colour = limitrofes)) +   geom_hline(yintercept = 0, color = "grey", alpha =0.7) + 
  ...
  labs(y = "en millones de US$", 
       x = "", 
       color= "",
       caption =  "**Fuente**: DMyE en base a datos de INDEC." )
...
texto_1 <- tabla_gasto_viaje_anual_pais_r %>% 
  filter(anio==anio_ref) %>% 
  select(starts_with("gasto")) %>%
  select(2:10) %>% 
  pivot_longer(cols = 1:9, names_to = "pais",
               values_to = "valor") %>% 
  arrange(desc(valor)) %>% 
  mutate(pais=str_sub(pais,15,nchar(pais)))
...
El gasto promedio por viaje de turista fue mayor en los residentes en `r texto_1[1,1]`, seguido por `r texto_1[2,1]` t `r texto_1[3,1]`.
...
ti10 <- gt(tabla_gasto_viaje_anual_pais_r) %>% 
 ...
 fmt_number(columns  = 1, sep_mark = "", decimals = 0) %>%  tab_source_note(source_note = md("**Fuente**: DMyE en base a datos de INDEC.")) %>% tab_options(container.height = 600,
              container.overflow.y = T) %>% 
...
Este descenso del gasto por viaje se explica por la caída en la estadía promedio del viaje, ya que el gasto promedio diario aumentó. 
...
La estadía promedio de los viajes de turistas no residentes en el año `r as.character(anio_ref)` fue de `r lbl_decimal(last(estadia_r$Total))` días, con una variación interanual de `r lbl_decimal(var_estad*100)`%. Cabe señalar que esta caída se debe a que 2022 fue un año atípico, aún afectado por efecto de la pandemia. 

En tanto, se observa un descenso de `r lbl_decimal(var_prepandemia_estad*100)`% al comparar con la estadía promedio del año 2019, la cual venía en línea con la tendencia histórica.
...
serie_vis_e <-excursionistas %>% 
  ...
  bind_rows(serie_vis_e_actual %>% filter(anio>=2016)) %>% 
  pivot_wider (names_from = tipo_visitante,
               values_from = tur) %>% 
  mutate (visitantes = Turistas + Excursionistas,
          var_interanual_vis = tasa_variacion(visitantes),
...
En el año `r as.character(anio_ref)` los viajes hacia países limítrofes vuelven a traccionar el crecimiento.
...
En el año `r as.character(anio_ref)`, `r g_1.3_texto_e[1,"destino_agrup"]` fue el destino principal del turismo emisivo, con `r lbl_int(pull(g_1.3_texto_e[1,"tur"]))` turistas 
...
```{r ti15, fig.cap= glue("Viajes de turistas residentes (en miles) por país de destino principal por año. Total país. Años {anio_ref-1}-{anio_ref}.")}

graf_1.2 <- ggplot(g_1.2, aes(x= destino_agrup, y = round(tur))) +   
geom_col(aes(fill = as_factor(anio)), position = "dodge") +
geom_label(aes(label = format(round(tur/1000,1), big.mark = ".", decimal.mark = ","), group = anio), 
           label.size = 0.15,
...
Al observar la evolución mensual del año `r as.character(anio_ref)` se puede ver una tendencia general de recuperación respecto a `r as.character(anio_ref)` (con la excepción del mes de octubre), con fuertes incrementos en el período enero-mayo. Al comparar con el año 2019, se observan caídas en todos los meses del año.

A lo largo de todo el año, **Brasil fue el destino principal.**
...
texto_1 <- via_pais_con_v19_e %>% 
  filter(destino_agrup=="Total") %>% 
  select(starts_with("prop")) %>% 
  pivot_longer(cols = 1:3, names_to = "indicador",
               values_to = "valor") %>% 
  arrange(desc(valor)) %>% 
...
Al desagregar la información por vía, se observa que en `r as.character(anio_ref)` para el turismo emisivo la principal vía fue la `r texto_1[1,1]` (concentrando el `r lbl_percent(as.numeric(texto_1[1,2]))`, con una var.i.a de `r lbl_percent(as.numeric(texto_2[1,2]))`), seguida por la vía `r texto_1[2,1]`.
...
Luego de la gran caída debido a la pandemia en 2020 y 2021, en 2022 comienza a ascender nuevamente hasta alcanzar valores pre-pandémicos en el año 2023. 

El gasto según destino limítrofe y no limítrofe ha sido relativamente parejo desde 1990, excepto en 2011 y 2013, donde es más alto en los primeros. En cambio, en 2018 y 2019 la diferencia se agranda en favor de los destinos no limítrofes. Aunque esto cambia en 2020, desde 2021 nuevamente el gasto en países no limítrofes vuelve a ser el más importante hasta 2023, año en el cual ambos gastos se equiparan.
...
ti22 <- gt(tabla_gasto_viaje_anual_pais_e) %>% 
 ...
 fmt_number(columns  = 1, sep_mark = "", decimals = 0) %>%  tab_source_note(source_note = md("**Fuente**: DMyE en base a datos de INDEC.")) %>% 
  tab_options(container.height = 600,
              container.overflow.y = T) %>% 
...
La caída internanual en el gasto por turista se explica tanto por la caída de la estadía promedio del viaje, como por el descenso del gasto promedio diario (`r lbl_decimal(last(gpd_e$var_tot)*100)` % interanual). En tanto, con respecto a 2019, tuvo una variación de `r lbl_decimal((var_prepandemia_gd_e*100))`%. 
...
texto <- gpd_e %>% 
  ...
  pivot_longer(cols=1:9,
               names_to = "pais",values_to = "valor") %>% 
  arrange(desc(valor))

```

El gasto promedio diario fue mayor en los que tuvieron como destino principal `r texto[1,1]`, seguidos por `r texto[2,1]` y `r texto[3,1]`.
...
La estadía promedio de los viajes de turistas residentes en el año `r as.character(anio_ref)` fue de `r lbl_decimal(last(estadia_e$Total))` días, con una variación interanual de `r lbl_decimal(var_estad_e*100)`%  y de `r lbl_decimal(var_prepandemia_estad_e*100)`% respecto a la  estadía de 2019. 
...
Las estadías en `r texto[1,1]`, `r texto[2,1]` y, en menor medida, `r texto[3,1]`,  han sido las más prolongadas (los países más lejanos suelen presentar históricamente estadías más largas).
...
ti25<- gt(tabla_balance_anual_pais) %>% 
  ...
  tab_source_note(
    source_note = md("**Fuente**: DMyE en base a datos de INDEC.")) %>% 
  tab_options(container.height = 600,
              container.overflow.y = T) %>%
...
En ambos casos se destaca el nivel de la pronunciada caída desde 2015, alcanzando su pico en 2017 y ascendiendo posteriormente. En el año `r as.character(anio_ref)` se observa el mejor saldo de viajes de turistas desde el 2014 (aunque el mismo fue negativo), no sucede lo mismo con el balance de divisas.
...
La **Encuesta de Turismo Internacional** es un relevamiento implementado de manera conjunta entre la Subsecretaría de Turismo y el INDEC.
...
eti_e <- herramientas::read_file_srv("/srv/DataDNMYE/eti/bases/eti_nr_2009_2025.rds") %>% 
...
eti_a <- herramientas::read_file_srv("/srv/DataDNMYE/eti/bases/eti_a_2014_2025.rds") %>% 
...
etie1 %>%
  ...
  tab_source_note(
    source_note = md("**Fuente:** Encuesta de Turismo Internacional (ETI) - (DMyE e INDEC).")) %>%
...
etie2 <- eti_e_perfiles %>%
  filter((p3_3>=anio_ref-1 | p3_3==2019)  & paso_final=="Ezeiza y Aeroparque") %>% 
...
if (anio_ref %in%c(2021,2022,2023)) {
...
ggplot(data=etie7 %>% mutate(motivo_viaje=factor(motivo_viaje,
  ...
  labs(x = "",
       y = "",
       colour="",
       caption = "Fuente: Encuesta de Turismo Internacional (ETI) - (DMyE e INDEC)")+
  theme_minimal()+
...
A nivel total, la mayor proporción de turistas no residentes se alojó en  **"`r ranking_total[nrow(ranking_total),2]`"** (el `r format(round(1*ranking_total[nrow(ranking_total),3],1),decimal.mark = ",",big.mark = ".")`% del total de turistas); mismo tipo de alojamiento se destaca en Ezeiza y Aeroparque y en el Aeropuerto de Mendoza; mientras que en el Aeropuerto de Córdoba y en Cristo Redentor la mayor parte de los turistas se hospedó en casa de familiares o amigos; en tanto que en el Puerto de Buenos Aires se destacó la categoría "otro tipo de alojamientos".
...
etie10_total <- eti_e_perfiles %>%
  filter(p3_3==anio_ref | p3_3==anio_ref-1 | p3_3==2019) %>%
  group_by(p3_3) %>%
  summarise(viajeros=sum(viajeros*wpf,na.rm = TRUE),
...
La mayor proporción de gasto provino de los turistas hospedados en la categoría **"`r ranking_total[nrow(ranking_total),2]`"** (`r format(round(1*ranking_total[nrow(ranking_total),3],1),decimal.mark = ",",big.mark = ".")`% del gasto total).

Por paso, las categorías por concentrar el peso del gasto fueron las siguientes:

+ **Ezeiza y Aeroparque**: **"`r ranking_eya[nrow(ranking_eya),2]`"** (`r format(round(1*ranking_eya[nrow(ranking_eya),3],1),decimal.mark = ",",big.mark = ".")`% del gasto)

+ **Aeropuerto de Córdoba**: **"`r ranking_cor[nrow(ranking_cor),2]`"** (`r format(round(1*ranking_cor[nrow(ranking_cor),3],1),decimal.mark = ",",big.mark = ".")`%)

+ **Aeropuerto de Mendoza**: **"`r ranking_mza[nrow(ranking_mza),2]`"** (`r format(round(1*ranking_mza[nrow(ranking_mza),3],1),decimal.mark = ",",big.mark = ".")`%)

+ **Puerto de Buenos Aires**: **"`r ranking_pue[nrow(ranking_pue),2]`"** (`r format(round(1*ranking_pue[nrow(ranking_pue),3],1),decimal.mark = ",",big.mark = ".")`%)

+ **Cristo Redentor**: **"`r ranking_cto[nrow(ranking_cto),2]`"** (`r format(round(1*ranking_cto[nrow(ranking_cto),3],1),decimal.mark = ",",big.mark = ".")`%)
...
etie17a <- eti_e_perfiles %>%
  filter(p3_3==anio_ref) %>%  
  summarise(viajeros=sum(viajeros*wpf,na.rm = TRUE),
            espec_cultura=sum(p39_1*wpf,na.rm = TRUE),
            espacios_cult=sum(p39_2*wpf,na.rm = TRUE),
            tango=sum(p39_3*wpf,na.rm = TRUE),
            compras=sum(p39_4*wpf,na.rm = TRUE),
            evdepor=sum(p39_5*wpf,na.rm = TRUE),
            fauna=sum(p39_6*wpf,na.rm = TRUE),
            medicos=sum(p39_7*wpf,na.rm = TRUE),
            spa=sum(p39_8*wpf,na.rm = TRUE),
            aventura=sum(p39_9*wpf,na.rm = TRUE),
            clases=sum(p39_10*wpf,na.rm = TRUE),
            parques=sum(p39_11*wpf,na.rm = TRUE),
            nieve=sum(p39_12*wpf,na.rm = TRUE),
            vinos=sum(p39_13*wpf,na.rm = TRUE),
            foodie=sum(p39_14*wpf,na.rm = TRUE),
            vegie=sum(p39_15*wpf,na.rm = TRUE),
            lgtbtiq=sum(p39_16*wpf,na.rm = TRUE),
            nocturno=sum(p39_17*wpf,na.rm = TRUE),
            boda=sum(p39_18*wpf,na.rm = TRUE),
            remoto=sum(p39_19*wpf,na.rm = TRUE),
            senderismo=sum(p39_20*wpf,na.rm = TRUE),
            otras_natu=sum(p39_21*wpf,na.rm = TRUE),
            otras=sum(p39_22*wpf,na.rm = TRUE)) %>% 
  mutate(prop_espec_cultura=espec_cultura/viajeros,
         prop_espacios_cult=espacios_cult/viajeros,
         prop_tango=tango/viajeros,
         prop_compras=compras/viajeros,
         prop_evdepor=evdepor/viajeros,
         prop_fauna=fauna/viajeros,
         prop_medicos=medicos/viajeros,
         prop_spa=spa/viajeros,
         prop_aventura=aventura/viajeros,
         prop_clases=clases/viajeros,
         prop_parques=parques/viajeros,
         prop_nieve=nieve/viajeros,
         prop_vinos=vinos/viajeros,
         prop_foodie=foodie/viajeros,
         prop_vegie=vegie/viajeros,
         prop_lgtbtiq=lgtbtiq/viajeros,
         prop_nocturno=nocturno/viajeros,
         prop_boda=boda/viajeros,
         prop_remoto=remoto/viajeros,
         prop_senderismo=senderismo/viajeros,
         prop_otras_natu=otras_natu/viajeros,
         prop_otras=otras/viajeros) %>% 
  select(starts_with("prop_"))
  
  etie17a <- etie17a %>% 
      pivot_longer(cols=c(1:ncol(etie17a)),
               names_to = "actividad",
               values_to="peso") %>% 
  mutate(peso=(round(peso,3))*100) %>% 
  mutate(actividad=case_when(
    actividad=="prop_espec_cultura"~"Asistencia espectáculos cult.",
          actividad=="prop_espacios_cult"~"Asistencia espacios cult.",
          actividad=="prop_tango"~"Acts. Tango",
          actividad=="prop_compras"~"Compras",
          actividad=="prop_evdepor"~"Eventos deportivos",
          actividad=="prop_fauna"~"Obs. de aves/fauna",
          actividad=="prop_medicos"~"Trat. Médicos",
          actividad=="prop_spa"~"Visita termas, spa",
          actividad=="prop_aventura"~"Turismo aventura",
          actividad=="prop_clases"~"Aprendizaje",
          actividad=="prop_parques"~"Vista parques, áreas protegidas",
          actividad=="prop_nieve"~"Acts. Nieve",
          actividad=="prop_vinos"~"Enoturismo",
          actividad=="prop_foodie"~"Acts. Foodie",
          actividad=="prop_vegie"~"Comida vegana",
          actividad=="prop_lgtbtiq"~"Fest. LGTBTIQ+",
          actividad=="prop_nocturno"~"Actividades nocturnas",
          actividad=="prop_boda"~"Boda",
          actividad=="prop_remoto"~"Trabajo remoto",
          actividad=="prop_senderismo"~"Senderismo",
          actividad=="prop_otras_natu"~"Otras acts. Naturaleza",
          actividad=="prop_otras"~"Otras actividades")) %>% 
  arrange((peso)) %>% 
...
texto <- etie17a %>%
  mutate(actividad=case_when(actividad=="Asistencia espectáculos cult."~"la asistencia a espectáculos culturales", 
                             actividad=="Asistencia espacios cult."~"la asistencia a espacios culturales", 
                             actividad=="Acts. Tango"~"las actividades de tango",
                             actividad=="Compras"~"las compras",
                             actividad=="Eventos deportivos"~"la asistencia a eventos deportivos",
                             actividad=="Obs. de aves/fauna"~"las actividades de observación de aves/fauna",
                             actividad=="Trat. Médicos"~"los tratamientos médicos",
                             actividad=="Visita termas, spa"~"las visitas a termas, spa",
                             actividad=="Turismo aventura"~"el turismo aventura",
                             actividad=="Aprendizaje"~"la asistencia a clases",
                             actividad=="Vista parques, áreas protegidas"~"la concurrencia a parques o áreas protegidas",
                             actividad=="Acts. Nieve"~"las actividades de nieve",
                             actividad=="Enoturismo"~"las actividades vinculadas al vino",
                             actividad=="Acts. Foodie"~"las actividades relacionadas  a la gastronomía",
                             actividad=="Comida vegana"~"el consumo de comida vegana",
                             actividad=="Fest. LGTBTIQ+"~"la asistencia a festivales LGTBTIQ+",
                             actividad=="Actividades nocturnas"~"la actividades nocturnas",
                             actividad=="Boda"~"la asistencia a una boda",
                             actividad=="Trabajo remoto"~"el trabajo remoto",
                             actividad=="Senderismo"~"las actividades vinculadas al senderismo",
                             actividad=="Otras acts. Naturaleza"~"la realización de otras actividades en la naturaleza",
                             actividad=="Otras actividades"~"la realización de otras actividades"))

```

Al analizar las **actividades realizadas** en la Argentina, se destacaron "`r texto[nrow(texto),1]`" (realizada por el `r format(round(1*texto[nrow(texto),2],1),decimal.mark = ",",big.mark = ".")`% de los turistas), luego "`r texto[nrow(texto)-1,1]`" y  "`r texto[nrow(texto)-2,1]`".
...
etie18 <- eti_e_perfiles %>%
  ...
  etie18 <- etie18 %>% 
      pivot_longer(cols=c(1:ncol(etie18)),
               names_to = "actividad",
               values_to="peso") %>% 
  mutate(peso=(round(peso,3))*100) %>% 
  mutate(actividad=case_when(
...
En **Ezeiza y Aeroparque**, en primer lugar se posicionó **"`r texto[nrow(texto),1]`"** (`r format(round(1*texto[nrow(texto),2],1),decimal.mark = ",",big.mark = ".")`%), luego "`r texto[nrow(texto)-1,1]`" y "`r texto[nrow(texto)-2,1]`".
...
etie19 <- eti_e_perfiles %>%
  ...
  mutate(prop_espec_cultura=espec_cultura/viajeros,
         ...
         prop_otras_natu=otras_natu/viajeros,
         prop_otras=otras/viajeros) %>% 
  select(orig_eya,starts_with("prop_"))

etie19 <- etie19 %>% 
  pivot_longer(cols=c(2:ncol(etie19)),
               names_to = "actividad",
               values_to="peso") %>% 
  mutate(peso=(round(peso,3))*100) %>% 
  mutate(actividad=case_when(
...
etie20 <- etie20 %>% 
  pivot_longer(cols=c(1:ncol(etie20)),
               names_to = "actividad",
               values_to="peso") %>% 
  mutate(peso=(round(peso,3))*100) %>% 
  mutate(actividad=case_when(
...
En el **Aeropuerto de Córdoba** se destacó **"`r texto[nrow(texto),1]`"** (`r format(round(1*texto[nrow(texto),2],1),decimal.mark = ",",big.mark = ".")`%), seguidas por "`r texto[nrow(texto)-1,1]`" y "`r texto[nrow(texto)-2,1]`".
...
etie21 <- eti_e_perfiles %>%
  ...
  mutate(prop_espec_cultura=espec_cultura/viajeros,
         ...
         prop_otras_natu=otras_natu/viajeros,
         prop_otras=otras/viajeros) %>% 
  select(orig_cor,starts_with("prop_"))

etie21 <- etie21 %>% 
  pivot_longer(cols=c(2:ncol(etie21)),
               names_to = "actividad",
               values_to="peso") %>% 
  mutate(peso=(round(peso,3))*100) %>% 
  mutate(actividad=case_when(
...
etie22 <- etie22 %>% 
  pivot_longer(cols=c(1:ncol(etie22)),
               names_to = "actividad",
               values_to="peso") %>% 
  mutate(peso=(round(peso,3))*100) %>% 
  mutate(actividad=case_when(
...
En el **Aeropuerto de Mendoza**, se encuentran**"`r texto[nrow(texto),1]`"** en primer lugar (`r format(round(1*texto[nrow(texto),2],1),decimal.mark = ",",big.mark = ".")`%), le siguieron en importancia "`r texto[nrow(texto)-1,1]`" y "`r texto[nrow(texto)-2,1]`". 
...
etie23 <- eti_e_perfiles %>%
  ...
  mutate(prop_espec_cultura=espec_cultura/viajeros,
         ...
         prop_otras_natu=otras_natu/viajeros,
         prop_otras=otras/viajeros) %>% 
  select(orig_mdz,starts_with("prop_"))

etie23 <- etie23 %>% 
  pivot_longer(cols=c(2:ncol(etie23)),
               names_to = "actividad",
               values_to="peso") %>% 
  mutate(peso=(round(peso,3))*100) %>% 
  mutate(actividad=case_when(
...
etie24 <- etie24 %>% 
  pivot_longer(cols=c(1:ncol(etie24)),
               names_to = "actividad",
               values_to="peso") %>% 
  mutate(peso=(round(peso,3))*100) %>% 
  mutate(actividad=case_when(
...
En el **Puerto de Buenos Aires**, se ubicaron en primer lugar **"`r texto[nrow(texto),1]`"** (`r format(round(1*texto[nrow(texto),2],1),decimal.mark = ",",big.mark = ".")`%), seguidas por "`r texto[nrow(texto)-1,1]`" y "`r texto[nrow(texto)-2,1]`".
...
etie25 <- eti_e_perfiles %>%
  ...
  mutate(prop_espec_cultura=espec_cultura/viajeros,
         ...
         prop_otras_natu=otras_natu/viajeros,
         prop_otras=otras/viajeros) %>% 
  select(orig_puerto,starts_with("prop_"))

etie25 <- etie25 %>% 
  pivot_longer(cols=c(2:ncol(etie25)),
               names_to = "actividad",
               values_to="peso") %>% 
  mutate(peso=(round(peso,3))*100) %>% 
  mutate(actividad=case_when(
...
etie26 <- etie26 %>% 
  pivot_longer(cols=c(1:ncol(etie26)),
               names_to = "actividad",
               values_to="peso") %>% 
  mutate(peso=(round(peso,3))*100) %>% 
  mutate(actividad=case_when(
...
Finalmente, en el **Paso Internacional Cristo Redentor**, se destacaron, en primer lugar, **"`r texto[nrow(texto),1]`"** (`r format(round(1*texto[nrow(texto),2],1),decimal.mark = ",",big.mark = ".")`%), "`r texto[nrow(texto)-1,1]`" y "`r texto[nrow(texto)-2,1]`".
...
etie27 <- eti_e_perfiles %>%
  ...
  mutate(prop_espec_cultura=espec_cultura/viajeros,
         ...
         prop_otras_natu=otras_natu/viajeros,
         prop_otras=otras/viajeros) %>% 
  select(orig_cristo,starts_with("prop_"))

etie27 <- etie27 %>% 
  pivot_longer(cols=c(2:ncol(etie27)),
               names_to = "actividad",
               values_to="peso") %>% 
  mutate(peso=(round(peso,3))*100) %>% 
  mutate(actividad=case_when(
...
ggplot(data=etie27 %>% mutate (orig_cristo=factor(orig_cristo,levels =c("Chile",
  ...
  labs(title = "",
       ...
       y = "",
       colour="",
       caption = "Fuenet: Encuesta de Turismo Internacional (ETI) - (DMyE e INDEC).")+
  scale_fill_dnmye(reverse = TRUE)+
  theme_minimal()+
  theme(plot.caption  = element_markdown(hjust = 0),
        plot.subtitle=element_text(colour = dnmye_colores("gris oscuro")),
...
cabotaje <- eti_e_perfiles %>% 
  filter((p40_1_1>=1 & p40_1_1<=5) & p3_3==anio_ref) %>%
  group_by(paso_final) %>% 
  summarise(calif=sum(p40_1_1*wpf,na.rm = TRUE),
            nbase=sum(viajeros*wpf,na.rm = TRUE)) %>%
  ungroup() %>% 
  adorn_totals() %>% 
  mutate(cabotaje=calif/nbase) %>% 
  select(1,4)

larga_dist <- eti_e_perfiles %>% 
  filter((p40_1_2>=1 & p40_1_2<=5) & p3_3==anio_ref) %>%
  group_by(paso_final) %>% 
  summarise(calif=sum(p40_1_2*wpf,na.rm = TRUE),
            nbase=sum(viajeros*wpf,na.rm = TRUE)) %>%
  ungroup() %>% 
  adorn_totals() %>% 
  mutate(larga_dist=calif/nbase) %>% 
  select(1,4)

alq_autos <- eti_e_perfiles %>% 
  filter((p40_1_3>=1 & p40_1_3<=5) & p3_3==anio_ref) %>%
  group_by(paso_final) %>% 
  summarise(calif=sum(p40_1_3*wpf,na.rm = TRUE),
            nbase=sum(viajeros*wpf,na.rm = TRUE)) %>%
  ungroup() %>% 
  adorn_totals() %>% 
  mutate(alq_autos=calif/nbase) %>% 
  select(1,4)
...
calif <- left_join(calif,cabotaje)
calif <- left_join(calif,larga_dist)
calif <- left_join(calif,alq_autos)

etie28 <- calif %>% 
  pivot_longer(cols=c(2:ncol(calif)),
               names_to = "rubro",
               values_to="valor") %>% 
  mutate(rubro=case_when(rubro=="tinterno"~"Transporte interno",
                         ...
                         rubro=="cabotaje"~"Aéreo de cabotaje",
                         rubro=="larga_dist"~"Transporte interurbano",
                         rubro=="alq_autos"~"Alquiler de autos"),
         valor=round(valor,1)) 


rm(tinterno,alojamiento,gastro,info,higiene,seguridad,estadia,inout,cabotaje,larga_dist,alq_autos,calif)
...
Cuando se desagrega la información por paso, en la mayoría se observa que "estadía en general" presenta buenas valoraciones, también se destaca en **Mendoza, Puerto de Buenos Aires y Cristo Redentor** el puntaje de la **gastronomía.** 
...
ggplot(data=etie28 %>% mutate (rubro=factor(rubro,levels = ranking),
  ...
  labs(title = "",
       ...
       y = "",
       colour="",
       caption = "Encuesta de Turismo Internacional (ETI) - (DMyE e INDEC).")+
  scale_fill_dnmye(reverse = TRUE)+
  theme_minimal()+
  theme(plot.caption  = element_markdown(hjust = 0),
        plot.subtitle=element_text(colour = dnmye_colores("gris oscuro")),
...
etie29 <- eti_e_perfiles %>% 
  filter(p17_tablet %in% c(5,7,11,12,13,14,15,16,17,18,41,42,43,44) & p3_3==anio_ref) %>%
  group_by(paso_final) %>% 
  summarise(viajeros=sum(viajeros*wpf,na.rm = TRUE),
            pcio_cal=sum(p41a_1*wpf,na.rm = TRUE),
            naturaleza=sum(p41a_2*wpf,na.rm = TRUE),
            cultura=sum(p41a_3*wpf,na.rm = TRUE),
            gastro=sum(p41a_4*wpf,na.rm = TRUE),
            proximidad=sum(p41a_5*wpf,na.rm = TRUE),
            promos=sum(p41a_6*wpf,na.rm = TRUE),
            act_esp=sum(p41a_7*wpf,na.rm = TRUE),
            amigos=sum(p41a_8*wpf,na.rm = TRUE),
            diversidad=sum(p41a_9*wpf,na.rm = TRUE),
            invita=sum(p41a_10*wpf,na.rm = TRUE),
            otros=sum(p41a_11*wpf,na.rm = TRUE)) %>%
  ungroup() %>% 
  adorn_totals() %>% 
  mutate(pcio_cal=pcio_cal/viajeros,
         naturaleza=naturaleza/viajeros,
         cultura=cultura/viajeros,
         gastro=gastro/viajeros,
         proximidad=proximidad/viajeros,
         promos=promos/viajeros,
         act_esp=act_esp/viajeros,
         amigos=amigos/viajeros,
         diversidad=diversidad/viajeros,
         invita=invita/viajeros,
         otros=otros/viajeros) %>% 
  select(-viajeros)

etie29 <- etie29 %>% 
  pivot_longer(cols=c(2:ncol(etie29)),
               names_to = "aspecto",
               values_to="valor") %>% 
  mutate(aspecto=case_when(aspecto=="pcio_cal"~"Relación precio/calidad",
                         aspecto=="naturaleza"~"Diversidad naturaleza",
                         aspecto=="cultura"~"Identidad cultural",
                         aspecto=="gastro"~"Gastronomía",
                         aspecto=="proximidad"~"Proximidad",
                         aspecto=="promos"~"Promociones esp. del país",
                         aspecto=="act_esp"~"Actividades esp.",
                         aspecto=="amigos"~"Presencia amigos/flia",
                         aspecto=="diversidad"~"Inclusivo en temas de género",
                         aspecto=="invita"~"Argentina invita",
                         aspecto=="otros"~"Otros"),
...
para_ranking  <- etie29 %>% 
  filter(paso_final=="Total") %>% 
  arrange(valor) %>% 
  mutate(secuencia=as.numeric(seq(1:(nrow(etie29)/6)))) %>% 
...
texto <- etie29 %>% 
  mutate(aspecto=case_when(aspecto=="Relación precio/calidad"~"la relación precio-calidad del país",
                           ...
                           aspecto=="Identidad cultural"~"los valores culturales",
                           aspecto=="Gastronomía"~"la gastronomía",
                           aspecto=="Proximidad"~"la proximidad",
                           aspecto=="Promociones esp. del país"~"las promociones específicas que ofrece la Argentina",
                           aspecto=="Actividades esp."~"las actividades específicas en el país",
                           aspecto=="Presencia amigos/flia"~"la prescencia de amigos o familiares",
                           aspecto=="Inclusivo en temas género"~"la Argentina es país inclusivo en temas de género",
                           aspecto=="Argentina invita"~"el programa Argentina Invita",
                           aspecto=="Otros"~"otros motivos"))
...
Por último, en **Cristo Redentor**, se destacó como primer lugar visitado **`r etie34_v[nrow(etie34_v),1]`** (`r format(round(1*etie34_v[nrow(etie34_v),2],1),decimal.mark = ",",big.mark = ".")`% de los viajeros y `r format(round(1*etie34_p[nrow(etie34_p),2],1),decimal.mark = ",",big.mark = ".")`% de los pernoctes).
...
if (anio_ref %in%c(2021,2022,anio_ref)) {
...
etia3 <- eti_a_perfiles %>%
  filter((p3_3>=anio_ref-1 | p3_3==2019) & paso_final=="Aep. Córdoba") %>% 
...
etia4 <- eti_a_perfiles %>%
  filter((p3_3>=anio_ref-1 | p3_3==2019) & paso_final=="Puerto de Buenos Aires") %>% 
...
etia5 <- eti_a_perfiles %>%
  filter((p3_3>=anio_ref-1 | p3_3==2019) & paso_final=="Aep. Mendoza") %>% 
...
etia6 <- eti_a_perfiles %>%
  filter((p3_3>=anio_ref-1 | p3_3==2019) & paso_final=="Cristo Redentor") %>% 
...
En la **totalidad de los pasos relevados por la ETI**, se destacó como **primer motivo de viaje** **"`r ranking_total[nrow(ranking_total),2]`"** (el `r format(round(1*ranking_total[nrow(ranking_total),3],1),decimal.mark = ",",big.mark = ".")`% del total). Dicho motivo **se repitió como el de mayor relevancia en el resto de los pasos relevados**, con la excepción del Puerto de Buenos Aires (donde se destacó "visita a familiares o amigos").
...
Cuando se diferencia por paso, en **Ezeiza y Aeroparque**, el **Aeropuerto de Córdoba**, el **Aeropuerto de Mendoza"** y en **Cristo Redentor** la mayor proporción de las pernoctaciones se generaron por turistas que viajaron por **vacaciones u ocio**; mientras que en **Puerto de Buenos Aires** se destaca "visita a "familiares o amigos". 
...
Tanto a nivel total, sobre el total de las pernoctaciones, **fue marginal el aporte de los turistas que contrataron paquete turístico para organizar su viaje al exterior**; en tanto que en el **Aeropuerto de Córdoba y el de Mendoza** el peso de los que utilizaron paquetes es considerablemente superior al del resto de los pasos.
...
Los datos que se muestran en este capítulo forman parte del Sistema de Información Turística de la Argentina (SINTA) <https://www.yvera.tur.ar/sinta/>{target="_blank"} de la Dirección de Mercados y Estadísticas (DNMyE). Los mismos se presentan a través de distintos formatos:
