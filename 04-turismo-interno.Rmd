...
En este capítulo se presenta un resumen de los principales indicadores del turismo interno de la Argentina, las caracterísiticas de las visitas y los principales resultados del comportamiento turístico en `r as.character(Anio)` (la proporción de las personas que han realizado al menos un viaje con pernocte en `r as.character(Anio)`). 

Los datos presentados provienen de la Encuesta de Viajes y Turismo de los Hogares (EVyTH), un operativo telefónico contínuo desde 2012 que encuesta mensualmente a los hogares de los grandes aglomerados urbanos del país donde reside casi dos tercios de la población total (64%); es la única fuente del país que releva información a nivel federal sobre turismo interno con rigurosidad técnica estadística (siguiendo las recomendaciones internacionales para la realización de estadísiticas de turismo doméstico). El tamaño muestral es de 2600 encuestas efectivas a hogares. Los resultados que se presentan son trimestrales, cuyos datos se pueden encontrar en el portal de datos abiertos. 

El objetivo de la EVyTH es medir y caracterizar el flujo y el gasto de los viajes turísticos (viajes con pernocte y visitas de un día o excursiones) que las personas residentes en Argentina realizan tanto dentro del país (turismo interno) como al exterior (turismo emisivo). En conjunto, los viajes turísticos relevados dan cuenta del turismo nacional. Entre sus objetivos la EVyTH permite computar y conocer aspectos socio-demográficos de quienes realizan los viajes y excursiones, posibilitando realizar perfiles de los visitantes y vislumbrar sus características estructurales.

Gracias a la continuidad temporal en el desarrollo de la encuesta, es posible la observación sostenida de la dinámica del turismo en el país y la población que lo realiza.
...
evyth <- read_file_srv("/srv/DataDNMYE/evyth/base_trabajo/evyth_base_de_trabajo.parquet") %>% 
  filter(anio %in% c(2023, 2022, 2021, 2019, 2015))#MODIFICAR MANUAL CUANDO CAMBIA ANIO
...
total <- data_tabla1 %>% 
  ...
  summarise(`2015` = sum(`2015`),
            ...
            `2022` = sum(`2022`),
            `2023` = sum(`2023`)) %>% #MODIFICAR MANUAL CUANDO CAMBIA ANIO
...
tabla1 <- tabla1 %>% 
  ...
  var_ia = (`2023`/`2022`)-1,#MODIFICAR MANUAL CUANDO CAMBIA ANIO
  var_ia19 = (`2023`/`2019`)-1)#MODIFICAR MANUAL CUANDO CAMBIA ANIO
...
tabla1_def <- tabla1 %>% 
  ...
  gt() %>% 
   cols_label(
    categoria = "",
    unidad = "",
    var_ia = "Var. i.a. % 2022",#MODIFICAR MANUAL CUANDO CAMBIA ANIO
  ...
  fmt_number(columns = c(`2015`:`2022`), rows = categoria == "Estadía promedio",#actualicé año anterior
             decimals = 1, dec_mark = ",") %>% 
  gt_theme_dnmye() %>% 
  tab_caption(caption = md(glue("Visitantes, estadía promedio, pernoctes y gasto con destino en Argentina por año. Años 2015/2019/2021/{anio_anterior}/{Anio}")))#MODIFICAR MANUAL CUANDO CAMBIA ANIO 
...
data_var <- evyth %>% 
  filter(tipo_visitante == 1, anio >= anio_anterior)#MODIFICAR MANUAL CUANDO CAMBIA ANIO  
