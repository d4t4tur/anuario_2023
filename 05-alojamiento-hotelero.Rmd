...
La siguiente sección se elaboró con los datos extraídos del Padrón Único Nacional de Alojamiento (PUNA) que elabora anualmente la Dirección de Mercados y Estadística (DMyE) a partir de la información aportada por los organismos de turismo provinciales y municipales y el INDEC.
...
puna1 <- puna %>% 
  ...
  labs(x = "", y = "", caption = "Fuente: DMyE en base a datos del PUNA") +
...
puna2 <- data_graph2 %>% 
  ...
  scale_fill_dnmye() +
  labs(fill = "", x = "", y = "", caption = "Fuente: DMyEen base a datos del PUNA") +
  theme_minimal() +
...
tabla1 <- data_cuadro1 %>% 
  ...
  cols_label(
    ...
    establecimientos_2023  = md("**Establecimientos**") ,
    plazas_2023  = md("**Plazas**"),
    var_ia_est_2023 = md("**Establecimientos**"),
    var_ia_pla_2023 = md("**Plazas**")
  ) %>% 
  ...
  cols_align(
    ...
    columns = c(establecimientos_2015, plazas_2015,
                establecimientos_2023, plazas_2023,
                var_ia_est_2023, var_ia_pla_2023))  %>% 
  tab_source_note(
    source_note = md(
      "**Fuente**: DMyEen base a datos del PUNA")
  )  %>%
  tab_spanner(
    label = md("**2015**"),
    columns = c(establecimientos_2015, plazas_2015)) %>%
  tab_spanner(
    label = md("**2023**"),
    columns = c(establecimientos_2023, plazas_2023)) %>%
  tab_spanner(
    label = md("**Var % i.a. 2023/2015**"),
    columns = c(var_ia_est_2023, var_ia_pla_2023)) %>%
...
data_cuadro2 <- rbind(total, data_cuadro2, totales) %>% 
  pivot_wider(names_from = "anio", 
              values_from = c("establecimientos", "plazas","var_ia_est","var_ia_pla")) %>% 
  select(-c(var_ia_est_2022, var_ia_pla_2022))
...
tabla2 <- data_cuadro2 %>% 
  ...
  tab_source_note(
    source_note = md(
      "**Fuente**: DMyE en base a datos del PUNA")
  ) %>%
  tab_spanner(
    label = md("**2022**"),
    columns = c(establecimientos_2022, plazas_2022)) %>%
...
puna6 <- ggplot(data_graph4, aes(str_wrap(region, 10), value)) +
  ...
  scale_y_percent(expand = expansion(mult = c(0.1, 0.2))) +
  scale_fill_dnmye() +
  labs(fill = "", x = "", y = "", caption = "Fuente: DMyE en base a datos del PUNA") +
  theme_minimal() +
  theme(legend.position = "bottom")
...
punaAnio <- herramientas::read_file_srv("/srv/DataDNMYE/puna/procesamiento/puna2023.xlsx")
...
puna10 <- total %>% 
  ...
  theme_minimal() +
  theme(legend.position = "bottom") +
  labs(x = "", y = "", fill = "", caption = "Fuente: DMyE en base a datos del PUNA")
...
ranking_cadAnt <- herramientas::read_file_srv("/srv/DataDNMYE/puna/procesamiento/puna2022.xlsx") %>% 
  ...
  ungroup() %>% 
  select(-plazas) %>% 
  mutate(cadena_hotelera = case_when(
    cadena_hotelera == "amerian" ~ "amerian hoteles",
    TRUE ~ cadena_hotelera))
...
eoh1 <- eoh %>%
  ...
  mutate(var_1 = round((pern_totales/lag(pern_totales,1) - 1),3),
         ...
         var_1b = round((pern_totales/lag(pern_totales, 4)-1),3), #Actualizar
         var_2b = round((pern_res/lag(pern_res, 4)-1),3), #Actualizar
         var_3b = round((pern_nores/lag(pern_nores, 4)-1),3), 
...
eoh1 %>%
  ...
  tab_source_note(
    source_note = md(
      "**Fuente:** Encuesta de Ocupación Hotelera (EOH) - (DMyE e INDEC)."))%>%
...
eoh2 <- eoh %>%
  ...
  mutate(var_1=round((viaj_totales/lag(viaj_totales,1)-1),3),
         ...
         var_1b=round((viaj_totales/lag(viaj_totales, 4)-1),3),#Actualizar
         var_2b=round((viaj_res/lag(viaj_res, 4)-1),3),#Actualizar
         var_3b=round((viaj_nores/lag(viaj_nores, 4)-1),3),#Actualizar
...
eoh3 <- eoh %>%
  filter(anio==anio_ref | anio==(anio_ref-1) | anio==2019) %>% 
  group_by(anio) %>% 
  summarise(pern_totales=sum(p5_t2*wr,na.rm = TRUE),
            pern_res=sum(p5_tr2*wr,na.rm = TRUE),
            pern_nores=sum(p5_tn2*wr,na.rm = TRUE),
...
```{r eoh3, fig.cap=glue("Pernoctaciones y viajeros (en millones) hospedados en establecimientos hoteleros y parahoteleros por condición de residencia. Total país. Años {2019}-{anio_ref}.")}

# Gráfico en ggplot

ggplot(data=eoh3 %>% 
         filter(residencia !="Total")
       %>% mutate(anio=factor(anio,levels = c(2019,anio_ref-1,anio_ref))),aes(x=anio,
                                                                              y=valor,
                                                                              fill=residencia,
  ...
  labs(title = "",
       ...
       y = "",
       colour="",
       caption = "Fuente: Encuesta de Ocupación Hotelera (EOH) - (DMyE e INDEC).") +
...
if (anio_ref %in% c(2022,2023)) { # Actualizar
  eoh4 <- eoh4 %>% filter(anio==anio_ref | anio==(anio_ref-1) | anio==(2019))
} else {
  eoh4 <- eoh4 %>% filter(anio==anio_ref | anio==(anio_ref-1) | anio==(2019))
}
...
```{r eoh4, fig.cap=glue("Estadía promedio (en noches) de los viajeros por año, según condición de residencia. Años {2019}-{anio_ref}.")}

#Estadía promedio de los viajeros por año, según condición de residencia.

#Gráfico en ggplot:

ggplot(data = eoh4 %>% mutate(anio=factor(anio,levels = c(2019, anio_ref-1,anio_ref)),
...
if (anio_ref %in% c(2022,2023)) {
  eoh16 <- eoh16 %>% filter(anio==anio_ref | anio==(anio_ref-1) | anio==(2019))
} else {
...
```{r eoh16, fig.cap=glue("Pernoctaciones y viajeros hospedados (en millones) en establecimientos hoteleros y parahoteleros por categoría del establecimiento. Años {2019}-{anio_ref}.")}
...
ggplot(data = eoh16%>% 
         mutate(anio=factor(anio,levels = c(2019,anio_ref-1,anio_ref)),
...
eoh17_hoteles <- eoh %>%
  filter((anio==anio_ref | anio==(anio_ref-1) | anio==(2019)) & categoria_del_hotel!="Para Hoteles") %>% 
  group_by(anio) %>% 
  summarise(pern_totales=sum(p5_t2*wr,na.rm = TRUE),
            pern_res=sum(p5_tr2*wr,na.rm = TRUE),
            pern_nores=sum(p5_tn2*wr,na.rm = TRUE),
...
eoh18 <- eoh %>%
  ...
  mutate(var_1=round((establecimientos/lag(establecimientos,1)-1),3),
         ...
         var_1b=round((establecimientos/lag(establecimientos,4)-1),3),
         var_2b=round((plazas_disp/lag(plazas_disp,4)-1),3),
...
Los datos que se muestran en este capítulo forman parte del Sistema de Información Turística de la Argentina (SINTA) <https://www.yvera.tur.ar/sinta/>{target="_blank"} de la Dirección de Mercados y Estadística (DNMyE). Los mismos se presentan a traves de distintos formatos:
