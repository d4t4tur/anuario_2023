...
Este capítulo presenta la evolución anual de las visitas de turistas a los Parques Nacionales en el país. A partir de información del Registro Nacional de Autorizaciones, Recaudaciones e Infracciones (RENARI) y de las Intendencias de las distintas áreas protegidas, la Administración de Parques Nacionales recopila y procesa los datos de los visitantes. Dicha información permite la clasificación de las visitas de los turistas en residentes en el país y no residentes en 38 Parques Nacionales en Argentina[^parque-patagonia].

[^parque-patagonia]: A partir del 2023, la APN incorporó información de visitas de los Parques Nacionales Campo San Juan (provincia de Misiones), Islas de Santa Fé (provincia de Santa Fé) y la Reserva Natural Silvestre Marismas del Tuyú (provincia de Buenos Aires), totalizando 41 Parques Nacionales y Reservas que producen información sobre sus visitantes. Esta información se publicará en el Anuario Estadístico de Turismo 2024 con su correspondiente variación interanual.
...
parques_nacionales <- herramientas::read_file_srv("/srv/DataDNMYE/areas_protegidas/areas_protegidas_nacionales/pivot_pn.xlsx", sheet= 2) %>%
  ...
  filter(!parque_nacional %in% 
             c("nahuel huapi", "campo san juan", "islas de santa fe", "campos del tuyu"))%>% 
  ...
  mutate(    parque_nacional = case_when(parque_nacional == "los glaciares" ~ "Los Glaciares",
                                ...
                                parque_nacional == "isla pingüino" ~ "Isla Pingüino",
                                parque_nacional == "pizarro" ~ "Pizarro",
                                # parque_nacional == "campo san juan" ~ "Campo San Juan",
                                # parque_nacional == "campos del tuyu" ~ "Campos del Tuyú",
                                # parque_nacional == "islas de santa fe" ~ "Islas de Santa Fé",
...
grafico1 <- datos_grafico1 %>% 
  ...
  labs(x="", fill = "", y  = "en miles",
       caption = "DMyE en base a datos suministrados por la Dirección de Mercadeo de la Dirección Nacional de Uso \nPúblico de la Administración de Parques Nacionales (APN).")
```
...
tabla_1 %>% 
  ...
  tab_footnote(
   ...
   tab_footnote(
    footnote = "Dato provisorio.",
   locations = cells_body(columns = anio, rows = anio== 2023), placement = "right"  )
...
tabla_regiones <- rbind(datos_regiones, total_region, total_pais) %>%
  ...
  mutate(var_total = (total_2023/total_2022)-1,
         var_res = (residentes_2023/residentes_2022)-1,
         var_nores = (no_residentes_2023/no_residentes_2022)-1,
         region = str_to_sentence(region),
         var_total_23_19 = (total_2023/total_2019)-1,
         var_res_23_19 = (residentes_2023/residentes_2019)-1,
         var_nores_23_19 = (no_residentes_2022/no_residentes_2019)-1) 
...
tabla_2 <- tabla_regiones %>% 
  select(region, parque_nacional, total_2023, residentes_2023, no_residentes_2023,
         var_total, var_res, var_nores,
         var_total_23_19,var_res_23_19, var_nores_23_19
  ...
  group_by(region) %>% 
  arrange(match(parque_nacional, c("Total país","Total región")), desc(total_2023)) %>% 
  ungroup() %>% 
  ...
  cols_label(
    ...
    total_2023 = md("**Total**"),
    residentes_2023 = md("**Residentes**"),
    no_residentes_2023 = md("**No residentes**"),
    var_total = md("**Total**"),
    ...
    var_total_23_19 = md("**Total**"),
    var_res_23_19 = md("**Residentes**"),
    var_nores_23_19 = md("**No residentes**"),                       
      ) %>% 
  ...
  tab_spanner(
    ...
    columns = c(var_total,
                   ...
                   var_nores)) %>%
  tab_spanner(
    label = md("**Var 2023/2019**"),
    columns = c(var_total_23_19,
                   var_res_23_19,
                   var_nores_23_19)) %>%
...
tabla_regiones1 <- rbind(datos_regiones, total_region, total_pais) %>%
  arrange(region, parque_nacional, anio) %>%
  pivot_wider(names_from = anio, values_from = c(residentes, no_residentes, total)) %>% 
  mutate(var_total = (total_2023/total_2019)-1,
         var_res = (residentes_2023/residentes_2019)-1,
         var_nores = (no_residentes_2023/no_residentes_2019)-1,
         # region = str_to_sentence(region)) %>% 
...
Al comparar los resultados del `r as.character(Anio)` con el `r as.character(Anio - 4)` (prepandemia), las visitas `r ifelse(totalreg>0, "registraron un incremento del", "tuvieron una caída del")` `r str_remove(lbl_percent(totalreg), "-")`, los residentes `r ifelse(totalreg>0, "registraron un incremento del", "tuvieron una caída del")` `r str_remove(lbl_percent(vartextreg),"-")` y los no residentes `r ifelse(totalreg>0, "registraron un incremento del", "tuvieron una caída del")` `r str_remove(lbl_percent(vartextreg_nores),"-")`, no alcanzando aún el máximo histórico registrado en el 2019. 

En `r as.character(Anio)` los parques de la región Patagonia (14 áreas protegidas) concentraron el 48,8% del total de las visitas a los parques nacionales del país (1,7 p.p. menos que en el `r as.character(Anio - 1)` y 1,4 p.p. menos si se lo compara con el `r as.character(Anio - 4)`), mientras que los parques de la región Litoral (10 áreas protegidas) totalizaron un 43,1% (1,3 p.p. más que en el `r as.character(Anio -1)` y 0,2 p.p. menos respecto del `r as.character(Anio - 4)`). Por otro lado, al observar la distribución según condición de residencia, en el `r as.character(Anio)` las visitas de los residentes tuvieron una participación del 64,7% y del 69,2% en las regiones Patagonia y Litoral respectivamente; la participación de las visitas de los no residentes fue de 35,3% y 30,9%, volviendo a los valores prepandémicos. 

```{r parques3}

tabla_2 %>% 
  sub_zero(columns = c(total_2023 , residentes_2023, no_residentes_2023)
           # total_2021, residentes_2021, no_residentes_2021,
           # total_2020, residentes_2020, no_residentes_2020, 
           # total_2019, residentes_2019, no_residentes_2019),
           ,
  ...
  tab_footnote(
    footnote = "Datos provisorios",
     locations = cells_column_spanners(spanners = c("**2023**")), placement = "right"
  ) %>%
    tab_footnote(
    footnote = "No se recibió información de visitas de los parques Lago Puelo, Los Glaciares (portada Lago Viedma), Lanín (portada Huechulaufquen en agosto), Nahuel Huapi (portada Puerto Pañuelo en julio). Los PN Perito Moreno e Isla Pingüino de abril a septiembre y de mayo a octubre respectivamente.",
  ...
  tab_footnote(
    ...
    tab_footnote(
    footnote = "No se recibió información de la RN Pizarro de febrero a mayo y de septiembre a diciembre del 2023.",
    locations = cells_row_groups(groups = c("Norte")), placement = "right"
  ) %>% 
   tab_footnote(
    footnote = "La visita en el PN Iberá se vio afectada por los incendios que afectaron la región en enero. En diciembre, la visita en el PN Pre Delta se cerró a la visita por una creciente del Río Paraná; no se recibió información de la RN Formosa (enero y febrero y de junio a diciembre), RNE Colonia Benitez (enero) y del PN Río Pilcomayo (potada Estero Poí en noviembre).",
    locations = cells_row_groups(groups = c("Litoral")), placement = "right"
  ) %>% 
   tab_footnote(
    footnote = "En noviembre PN Ciervo de los Pantanos se cerró a la visita por obras).",
    locations = cells_row_groups(groups = c("Buenos aires")), placement = "right"
  )
...
Al considerar las visitas según condición de residencia, se observa `r ifelse(vartext2 > 0, "el incremento del", " una caída de")` `r lbl_percent(vartext2)` en las visitas de los residentes que se explica principalmente por `r ifelse(vartexpatag>0, "un crecimiento de", "tuvieron una caída del")` las visitas de los parques de las regiones Patagonia y Litoral (`r lbl_percent(vartexpatag)` y `r lbl_percent(vartexlitoral)` respectivamente. 

En cuanto a las visitas de los no residentes, `r ifelse(vartext3 > 0, "el incremento del", " una caída de")` `r lbl_percent(vartext3)`, se explica por la progresiva recuperación del turismo receptivo luego de la pandemia del Covid-19, ubicándose en segundo lugar respecto del máximo histórico que se alcanzó en 2019.
...
datos_grafico2 %>% 
  ...
  labs(x = "", y = "", fill = "",
       caption = "Fuente: DMyE en base a datos de la APN.") +
  theme_minimal() +
  theme(legend.position = "bottom",
        #text = element_text(family = "Encode Sans"),
...
tabla_regiones_trim <- rbind(total_pais, total_pais_trim, total_region_trim, datos_regiones_trim) %>%
  ...
  mutate(var_total = (total_2023/total_2022)-1,
         ...
         region = str_to_sentence(region)) %>% 
  group_by(region) %>% 
  mutate(total_trim_reg = sum(total_2023)/2,
         res_trim_reg = sum(residentes_2023)/2,
         nores_trim_reg = sum(no_residentes_2023)/2) %>% 
  ungroup() %>% 
  mutate(part_total = total_2023/total_trim_reg,
         part_res = residentes_2023/res_trim_reg,
         part_nores = no_residentes_2023/nores_trim_reg)
...
tabla_3 <- tabla_regiones_trim %>% 
  select(region, trimestre, total_2023, residentes_2023, no_residentes_2023,
         var_total, var_res, var_nores,
...
En el primer trimestre `r as.character(Anio)` `r ifelse(var1t > 0, "se registró un incremento del", " se registró una caída de")` `r str_remove(lbl_percent(var1t),"-")`, propiciada por una caída en las visitas en la región Patagonia respecto del  `r as.character(Anio - 1)`.
En el segundo y cuarto trimestre se registraron en el país los mayores incrementos en las visitas respecto del mismo período del año anterior (`r lbl_percent(var2t)` y  `r lbl_percent(var4t)` respectivamente).  
...
Al observar la participación de los 10 parques con mayor volumen de visitas en el `r as.character(Anio)`, el 76,4% se concentró en cuatro parques nacionales: el PN Iguazú `r ifelse(partmax > 0, "fue el que concentró el mayor número de visitas", " fue el que concentró el menor número de visitas") ` `r lbl_percent(partmax)`
, siguiéndole en importancia tres parques de la región Patagonia:
Los Glaciares, Nahuel Huapi y Tierra del Fuego, que reunieron en conjunto el 39,8% del total.
...
En el  `r as.character(Anio)` la mayor proporción de visitas que recibieron los Parques Nacionales fueron de turistas residentes. En cuanto a los turistas no residentes, el PN `r parquemax_nores` fue el que tuvo la mayor participación de turistas no residentes (`r lbl_percent(partmax_nores)`), mientras que en los PN `r parquemax_nores2` y `r parquemax_nores3`, la participación de turistas no residentes fue de `r lbl_percent(partmax_nores2)` y `r lbl_percent(partmax_nores3)` de las visitas totales del `r as.character(Anio)`.
...
Los datos que se muestran en este capítulo forman parte del Sistema de Información Turística de la Argentina (SINTA) <https://www.yvera.tur.ar/sinta/>{target="_blank"} de la Dirección de Mercados y Estadísticas (DMyE). Los mismos se presentan a través de distintos formatos:
...
-   [Tablero:](https://tableros.yvera.tur.ar/tablero_areas_protegidas/){target="_blank"}Tablero interactivo con información mensual de visitas por Área Protegida.
