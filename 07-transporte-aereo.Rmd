...
freq_cordoba_loc <- freq_reg_prov_loc[4,3] %>% pull
freq_cordoba <- freq_reg_prov_loc[4,16] %>% 
  pull %>%
  format(., big.mark = ".")

freq_cuyo_loc <- freq_reg_prov_loc[3,3] %>% pull
freq_cuyo <- freq_reg_prov_loc[3,16] %>%
  pull %>%
  format(., big.mark = ".")
...
freq_ciudad_dest_actual <- freq_ciudad_dest_actual %>% 
  ...
  group_by(destino_localidad_etiqueta) %>% 
  summarise(frecuencias_actual = sum(frecuencias_actual, na.rm = T),
            frecuencias_anterior = sum(frecuencias_anterior, na.rm = T),
            frecuencias_base = sum(frecuencias_base, na.rm = T),
...
ciudad_int_orig_primer_frq <- (internacional_x_ciudad %>% filter(origen_localidad_etiqueta != "Resto"))[1,2] %>% pull() %>% format(., big.mark = ".")
ciudad_int_orig_segund_frq <- (internacional_x_ciudad %>% filter(origen_localidad_etiqueta != "Resto"))[2,2] %>% pull() %>% format(., big.mark = ".")
ciudad_int_orig_tercer_frq <- (internacional_x_ciudad %>% filter(origen_localidad_etiqueta != "Resto"))[3,2] %>% pull() %>% format(., big.mark = ".")
...
vuelos_int_actual <- internacional %>% 
  ...
  arrange(-frecuencias_actual) %>% 
  mutate(posicion_actual = 1:n(),
         participacion_actual = round(frecuencias_actual/sum(frecuencias_actual, na.rm = T), 3))
...
vuelos_int <- vuelos_int_actual %>% 
  ...
  filter(!is.na(posicion_actual) & !is.na(posicion_anterior) & !is.na(posicion_base)) %>% 
  mutate(variacion = round((frecuencias_actual - frecuencias_anterior)/frecuencias_anterior, 3),
         variacion_base = round((frecuencias_actual - frecuencias_base)/frecuencias_base, 3),
...
tabla_10_aerocomercial <- internacional %>% 
  ...
  summarise(frecuencias = n()) %>%
  filter(sum(frecuencias, na.rm = T) >= 24,) %>% 
...
herfindah_aero_int_cond <- if_else((herfintal_aerolinea_int %>% mutate(across(.cols=c(2:4), ~ .*100)))[1,4] < (herfintal_aerolinea_int %>% mutate(across(.cols = c(2:4), ~ .*100)))[1,3],
                                   paste0("una reducción de ", round((herfintal_aerolinea_int %>% mutate(across(.cols=c(2:4), ~ .*100)))[1,4]-(herfintal_aerolinea_int %>% mutate(across(.cols=c(2:4), ~ .*100)))[1,3], 2)),
                                   paste0("un crecimiento de ", round((herfintal_aerolinea_int %>% mutate(across(.cols=c(2:4), ~ .*100)))[1,4]-(herfintal_aerolinea_int %>% mutate(across(.cols=c(2:4), ~ .*100)))[1,3], 2)))
